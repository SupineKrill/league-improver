...
class LoLAPI:
    ...
    def get_summoner_id(self,summoner_name, return_json=True):
        ...
        if self.real_name(summoner_name):
            return self.request_json(f"/lol/summoner/v3/summoners/by-name/{summoner_name}", return_json)

    def get_ranked_games(self, account_id, return_json=True):
        """Takes account ID. Returns JSON of all ranked games played"""
        return self.request_json(f"/lol/match/v3/matchlists/by-account/{account_id}", return_json) 

    def get_last_20(self, account_id, return_json=True):
            return self.request_json(f"/lol/match/v3/matchlists/by-account/{account_id}/recent", return_json)

if __name__ == '__main__':
    test = LoLAPI()
    self_id = test.get_summoner_id("멘탈잡자으쌰으쌰", True)
    print(self_id)
    print(test.get_last_20(self_id["accountId"]))